...
int main(int argc, const char *argv[]) {
  try {
    ...
    // It would be nice if a CLI's signal handler can be responsible for calling
    // fuzzer->ReceiveStopSignal() in the future

    // Now the fuzzing campaign begins
    // TODO: The timeout for the fuzzing campaign has not been implemented.
    // FIXME: fuzzer->ShouldEnd() seems always false when libfuzzer&nezha is
    // used
    while (!fuzzer->ShouldEnd()) {
      fuzzer->OneLoop();
      // Call hooks per OneLoop, if necessary
    }
  } catch (const fuzzuf::exceptions::fuzzuf_runtime_error &e) {
    ...
    return -1;  // Use exit code to indicate that the process has failed with an
                // error
  } catch (const fuzzuf::exceptions::fuzzuf_logic_error &e) {
    // CLI does error handling as the topmost module
    std::cerr << "[!] " << e.what() << std::endl;
    std::cerr << "\tat " << e.file << ":" << e.line << std::endl;
