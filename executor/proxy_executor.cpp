...
ProxyExecutor::ProxyExecutor(
    ...
    ProxyExecutor("", std::vector<std::string>(), argv, exec_timelimit_ms, exec_memlimit, path_to_write_input)
{
}
...
void ProxyExecutor::Initilize()
{
    ...
    // active_instance may refer other instance if multiple ProxyExecutor exist, therefore following assertion is not appliable.
    ...
    if (has_shared_memories) {
        ...
        // It is sufficient if each ProxyExecutor::Run() can refer the memory
        SetupSharedMemories();
...
//    - ProxyExecutor executes proxy_path directly, and expects that proxy on proxy_path executes argv[0] in some way.
//      * ProxyExecutor doesn't participate on how argv[0] is executed.
...
/**
 * Precondition:
 *  - input_fd has a file descriptor that is set by ProxyExecutor::SetupIO()
 * Postcondition:
...
void ProxyExecutor::Run(const u8 *buf, u32 len, u32 timeout_ms) {
    ...
    if (forksrv) {
        ...
        // The handler for SIGALRM is set to ProxyExecutor::AlarmHandler, and the PUT is killed in inside.
...
u32 ProxyExecutor::GetAFLMapSize() {
...
FileFeedback ProxyExecutor::GetFileFeedback(fs::path feed_path) {
