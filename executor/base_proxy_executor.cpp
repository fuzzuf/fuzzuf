...
//    - The derived class constructor is responsible for initializing cargv and stdin_mode.
...
// This constructor creates an executor without coverage feedbacks and fork server.
BaseProxyExecutor::BaseProxyExecutor(
    const fs::path &proxy_path,
    const std::vector<std::string> &pargv,
    const std::vector<std::string> &argv,
    ...
    BaseProxyExecutor(proxy_path, pargv, argv, exec_timelimit_ms, exec_memlimit, false, path_to_write_input)
{
}
...
BaseProxyExecutor::BaseProxyExecutor(
    ...
    BaseProxyExecutor("", std::vector<std::string>(), argv, exec_timelimit_ms, exec_memlimit, path_to_write_input)
{
}
...
void BaseProxyExecutor::Initilize()
{
    ...
    // active_instance may refer other instance if multiple BaseProxyExecutor exist, therefore following assertion is not appliable.
...
// Postcondition:
//    - Create command line arguments those are passed to proxy located at proxy_path.
//      * Create cargv in the form of `proxy_path pargv[] -- argv[]`
//    - BaseProxyExecutor executes proxy_path directly, and expects that proxy on proxy_path executes argv[0] in some way.
...
void BaseProxyExecutor::Run(const u8 *buf, u32 len, u32 timeout_ms) {
	...
	// The value tmp which is needed only on persistent mode that is currently not implemented.
        ...
        // The  new PUT execution can be requested to the fork server by writing 4 byte of values to the pipe.
    ...
    if (forksrv) {
        ...
        // The handler for SIGALRM is set to BaseProxyExecutor::AlarmHandler, and the PUT is killed in inside.
...
InplaceMemoryFeedback BaseProxyExecutor::GetStdOut() {
    return InplaceMemoryFeedback( stdout_buffer.data(), stdout_buffer.size(), lock);
}

InplaceMemoryFeedback BaseProxyExecutor::GetStdErr() {
    return InplaceMemoryFeedback( stderr_buffer.data(), stderr_buffer.size(), lock);
}

FileFeedback BaseProxyExecutor::GetFileFeedback(fs::path feed_path) {
    return FileFeedback(feed_path, lock);
}

ExitStatusFeedback BaseProxyExecutor::GetExitStatusFeedback() {
    return ExitStatusFeedback(last_exit_reason, last_signal);
}

bool BaseProxyExecutor::IsFeedbackLocked() {
...
// basically, we care about only the case where BaseProxyExecutor::Run is running.
// in that case, we should kill the child process(of PUT) so that BaseProxyExecutor could halt without waiting the timeout.
...
void BaseProxyExecutor::ReceiveStopSignal(void) {
    ...
    // the child process is active only in BaseProxyExecutor::Run and Run always uses waitpid, so we don't need to use waitpid here
    KillChildWithoutWait();
}
