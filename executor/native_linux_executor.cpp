...
NativeLinuxExecutor::NativeLinuxExecutor(
  ...
  // active_instance may refer other instance if multiple NativeLinuxExecutor
 ...
 * NativeLinuxExecutor::SetupIO() Postcondition:
...
void NativeLinuxExecutor::Run(const u8 *buf, u32 len, u32 timeout_ms) {
  ...
  if (forksrv) {
    ...
    // The new PUT execution can be requested to the fork server by writing
    // 4byte values to the pipe. If the PUT launched successfully, the pid of
    ...
    if (!child_pid) {
      ...
      child_state->exec_result =
          execve(cargv[0], (char **)cargv.data(),
  ...
  if (forksrv) {
    ...
    // The handler for SIGALRM is set to NativeLinuxExecutor::AlarmHandler, and
...
int NativeLinuxExecutor::GetAFLShmID() { return afl_edge_coverage.GetShmID(); }
...
// basically, we care about only the case where NativeLinuxExecutor::Run is
// running. in that case, we should kill the child process(of PUT) so that
// NativeLinuxExecutor could halt without waiting the timeout. if this function
// is called during the call of other functions, then the child process is not
...
void NativeLinuxExecutor::ReceiveStopSignal(void) {
  ...
  // the child process is active only in NativeLinuxExecutor::Run and Run always
