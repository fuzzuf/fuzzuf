...
VUzzerMidCalleeRef DecideKeep::operator()(void) {
    if (state.errorbb) {
        ...
        if (0 < state.loop_cnt && state.loop_cnt < state.gennum / 5 && state.loop_cnt % state.keepslide == 0) {
            std::sample(state.seed_queue.begin(),
                        state.seed_queue.end(),
                        std::back_inserter(state.keep_queue),
                        state.keepfilenum,
...
VUzzerMidCalleeRef RunEHB::operator()(void) {
    ...
    if (state.loop_cnt > 40 && state.loop_cnt % state.bbslide == 0) {
        ...
        */
        ...
        const u32 ratio = (state.setting->ehb_bb_ratio/100) * state.setting->pop_size;
        for (size_t i = 0; i < all_bb.size(); i++) {
            ...
            if (bb_appear > ratio && state.good_bbs.find(all_bb[i]) != state.good_bbs.end()) {
...
VUzzerMidCalleeRef ExecuteTaintPUT::operator()(void) {
    ...
    for (const auto& testcase : state.taint_queue) {
         ...
         * FIXME: Occasionally taint executor doesn't record any taint info. Both taint_cmp_offsets and taint_lea_offsets could become empty.
         */
