...
AFLplusplusState::AFLplusplusState(
    ...
    : afl::AFLStateTemplate<AFLplusplusTestcase>(setting, executor, std::move(mutop_optimizer)),
      setting(setting),
      prev_queued_items(0),
...
std::shared_ptr<AFLplusplusTestcase> AFLplusplusState::AddToQueue(const std::string &fn,
                                                          const u8 *buf,
                                                          u32 len,
                                                          bool passed_det) {
  std::shared_ptr<AFLplusplusTestcase> testcase =
      AFLStateTemplate<AFLplusplusTestcase>::AddToQueue(fn, buf, len, passed_det);

  return testcase;
}

void AFLplusplusState::UpdateBitmapScoreWithRawTrace(AFLplusplusTestcase &testcase,
                                                 const u8 *trace_bits,
                                                 u32 map_size) {
