...
State::State(
    ...
    : afl::AFLStateTemplate<Testcase>(setting, executor,
...
std::shared_ptr<Testcase> State::AddToQueue(const std::string &fn,
                                                        const u8 *buf, u32 len,
                                                        bool passed_det) {
  std::shared_ptr<Testcase> testcase =
      AFLStateTemplate<Testcase>::AddToQueue(fn, buf, len, passed_det);

  return testcase;
}

void State::UpdateBitmapScoreWithRawTrace(Testcase &testcase,
                                                const u8 *trace_bits,
                                                u32 map_size) {
  if constexpr ( afl::option::EnableKScheduler<Tag>() ) {
    ...
    for (i = 0; i < afl::option::GetMapSize<Tag>(); i++) {
...
bool State::SaveIfInteresting(const u8 *buf, u32 len,
  ...
  bool res = AFLStateTemplate<Testcase>::SaveIfInteresting(
...
double State::DoCalcScore(Testcase &testcase) {
