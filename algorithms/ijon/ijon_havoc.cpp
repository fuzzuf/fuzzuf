...
enum IJONExtraHavocCase : u32 {
  IJON_DELETE_BYTES = fuzzuf::mutator::NUM_CASE,  // directly follow HavocCase
  IJON_INSERT_EXTRA,
  IJON_INSERT_AEXTRA,
  IJON_NUM_CASE  // this represents the number of cases in IJON's havoc
};

static constexpr std::array<double, IJON_NUM_CASE> IJONGetCaseWeights(
    bool has_extras, bool has_aextras) {
  using afl::util::AFLGetCaseWeights;

  // IJON's probability distribution of mutation operators is based on that of
  // AFL.
  std::array<double, fuzzuf::mutator::NUM_CASE> base_weights =
      AFLGetCaseWeights(has_extras, has_aextras);
  ...
  // So we need to change the weights of these cases in the probability
  // distributions accordingly. See the followings:
  ...
  ret[IJON_DELETE_BYTES] = ret[fuzzuf::mutator::DELETE_BYTES];
  ret[fuzzuf::mutator::DELETE_BYTES] = 0.0;

  ret[IJON_INSERT_EXTRA] = ret[fuzzuf::mutator::INSERT_EXTRA];
  ret[fuzzuf::mutator::INSERT_EXTRA] = 0.0;

  ret[IJON_INSERT_AEXTRA] = ret[fuzzuf::mutator::INSERT_AEXTRA];
  ret[fuzzuf::mutator::INSERT_AEXTRA] = 0.0;
 ...
 * in the havoc mutation of IJON. Depending on whether IJON has a dictionary for
 * a PUT, the function employs different distributions.
 */
u32 IJONHavocCaseDistrib::CalcValue() {
  ...
  // FIXME: actually, libstdc++'s discrete_distribution doesn't use Walker's
  // alias method... We should implement it by ourselves...
  static std::discrete_distribution<u32> dists[2][2] = {
      {std::discrete_distribution<u32>(weight_set[0][0].begin(),
                                       weight_set[0][0].end()),
       std::discrete_distribution<u32>(weight_set[0][1].begin(),
                                       weight_set[0][1].end())},
      {std::discrete_distribution<u32>(weight_set[1][0].begin(),
                                       weight_set[1][0].end()),
       std::discrete_distribution<u32>(weight_set[1][1].begin(),
                                       weight_set[1][1].end())},
  };
...
void IJONCustomCases(
  ...
  switch (case_idx) {
    case IJON_DELETE_BYTES:
      ...
      // slightly increases the expected number of mutation operators applied at
      // once in havoc.
      break;

    case IJON_INSERT_EXTRA:
      [[fallthrough]];
