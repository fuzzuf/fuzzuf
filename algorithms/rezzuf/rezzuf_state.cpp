...
RezzufState::RezzufState(
    std::shared_ptr<const RezzufSetting> setting,
    std::shared_ptr<executor::AFLExecutorInterface> executor,
    std::unique_ptr<optimizer::HavocOptimizer> &&havoc_optimizer)
    : afl::AFLStateTemplate<RezzufTestcase>(setting, executor,
                                            std::move(havoc_optimizer)),
      setting(setting),
      prev_queued_items(0),
      alias_probability(nullptr) {
  using aflplusplus::option::GetNFuzzSize;
  n_fuzz.reset(new u32[GetNFuzzSize<Tag>()]);
}

std::shared_ptr<RezzufTestcase> RezzufState::AddToQueue(const std::string &fn,
                                                        const u8 *buf, u32 len,
                                                        bool passed_det) {
  std::shared_ptr<RezzufTestcase> testcase =
      AFLStateTemplate<RezzufTestcase>::AddToQueue(fn, buf, len, passed_det);

  return testcase;
}

void RezzufState::UpdateBitmapScoreWithRawTrace(RezzufTestcase &testcase,
                                                const u8 *trace_bits,
                                                u32 map_size) {
...
bool RezzufState::SaveIfInteresting(const u8 *buf, u32 len,
  ...
  bool res = AFLStateTemplate<RezzufTestcase>::SaveIfInteresting(
...
u32 RezzufState::DoCalcScore(RezzufTestcase &testcase) {
...
void RezzufState::ShowStats(void) {
