...
std::shared_ptr<AFLFastTestcase> AFLFastState::AddToQueue(
    ...
    std::shared_ptr<AFLFastTestcase> testcase = AFLStateTemplate<AFLFastTestcase>::AddToQueue(fn, buf, len, passed_det);
    testcase->n_fuzz = 1;

    return testcase;
}

void AFLFastState::UpdateBitmapScoreWithRawTrace(
...
u32 AFLFastState::DoCalcScore(AFLFastTestcase &testcase) {
    ...
    switch (setting->schedule) {
        ...
        case option::QUAD:
            factor = testcase.fuzz_level * testcase.fuzz_level / (fuzz == 0 ? 1 : fuzz);
            break;
