...
// FIXME: all the classes related to HierarFlow should be put under namespace fuzzuf::hierarflow
...
template<class HeadI, class HeadO, class TailI, class TailO, bool IS_TAIL_REGULAR>
...
/**
 * @details The wrapper of HierarFlowNodeImpl. In order to support operators on HierarFlowNodeImpl,
 *          HierarFlowNodeImpl should be always handled via std::shared_ptr. This means we should not expose
 *          HierarFlowNodeImpl itself to users because it may result in using HierarFlowNodeImpl without
 *          std::shared_ptr. Therefore this is the class actually the instance of which users can create.
 * @tparam IS_REGULAR Fuzzuf has special nodes, such as CallRandomChild, that act differently from usual
 *         nodes. For example, CallRandomChild randomly calls only one child instead of calling their
 *         children one by one. Also, it's possible that you and we will implement more other special nodes
 *         in the future. Because it would be confusing if we treat such nodes in the same way as usual
 *         nodes, we need to separate their type and the notation to use them.
 * @todo  We should duplicate HierarFlowRoutine and HierarFlowNodeImpl and define HierarFlowIrregularRoutine
 *        and HierarFlowIrregularNodeImpl likewise. Otherwise, we may treat irregular HierarFlowRoutines as
 *        regular ones by mistake, and currently no error would be emitted for that mistake.
 */
...
public:
    ...
    HierarFlowNode<I, O, IS_REGULAR>& operator=(HierarFlowNode<I, O, IS_REGULAR>&& orig) {
    ...
    auto operator<<(HierarFlowNode<O, O2, IS_SUCC_REGULAR> succ)
        -> std::enable_if_t< IS_REGULAR_, HierarFlowPath<I, O, O, O2, IS_SUCC_REGULAR> > {
    ...
    template<class T, class TailI, class TailO, bool IS_PATH_REGULAR, bool IS_REGULAR_=IS_REGULAR>
    auto operator<<(HierarFlowPath<O, T, TailI, TailO, IS_PATH_REGULAR> path)
        -> std::enable_if_t< IS_REGULAR_, HierarFlowPath<I, O, TailI, TailO, IS_PATH_REGULAR> > {
    ...
    auto operator<<(HierarFlowChildren<O, ChildHeadO, ChildTailO> children)
        -> std::enable_if_t< IS_REGULAR_, HierarFlowPath<I, O, O, ChildHeadO, true> > {
        ...
        if (children.IsInvalidInstance()) {
            throw exceptions::wrong_hierarflow_usage(
                ...
                "You should never use HierarFlowChildren instances twice by saving it as a variable.\n"
                "For example, `auto children = (a || b); c << (children || d); e << (children || e);` doesn't make sense.",
                __FILE__, __LINE__
            );
        }
        ...
        return HierarFlowPath<I, O, O, ChildHeadO, true>(impl, children.first_child);
    }
    ...
    // To emit human-readable compilation errors, we intentionally make static_assert fail
    // in prohibited(i.e. undefined in SFINAE) cases

    template<class... Args, bool IS_REGULAR_=IS_REGULAR>
    auto operator<<(Args...) -> std::enable_if_t< !IS_REGULAR_, void > {

        static_assert( IS_REGULAR_, "You cannot use operator<< with irregular nodes. Use operator<=." );
    }

    template<class... Args, bool IS_REGULAR_=IS_REGULAR>
    auto operator[](Args...) -> std::enable_if_t< !IS_REGULAR_, void > {

        static_assert( IS_REGULAR_, "You cannot use operator[] with irregular nodes. Use operator<= and ()." );
    }

    template<class... Args, bool IS_REGULAR_=IS_REGULAR>
    auto operator<=(Args...) -> std::enable_if_t< IS_REGULAR_, void > {

        static_assert( !IS_REGULAR_, "You cannot use operator<= with regular nodes. Use operator<< or operator[]." );
    }


    template<class O2, bool IS_NODE_REGULAR>
    HierarFlowChildren<I, O, O2> operator||(HierarFlowNode<I, O2, IS_NODE_REGULAR> brother) {
    ...
    HierarFlowChildren<I, O, HeadO> operator||(HierarFlowPath<I, HeadO, TailI, TailO, IS_PATH_REGULAR> path) {
    ...
    // For details, see the comment of WrapToMakeHeadNode.
    ...
    // Using this function a lot is not recommended. But some of utilities require this...
    std::shared_ptr<HierarFlowNodeImpl<I, O>> ShareImpl() {
        return impl;
    }
