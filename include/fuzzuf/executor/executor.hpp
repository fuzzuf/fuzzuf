...
public:
    ...
    std::vector<const char*> cargv; // argv converted to char*. Do not reference after the lifetime of argv ends as an address pointed by char* is obtained by c_str()

    // A filepath given to the PUT which reads an input over the file (note that Executor creates a file to the path)
    ...
    int null_fd;
    bool stdin_mode; // FIXME: Maybe use struct NativeLinuxExecutorArgs{cargv, stdin_mode} and make is const
    ...
    // Called when fuzzuf needs to be stopped earlier, such as when SIGTERM is triggered
