...
struct AFLplusplusFuzzerOptions {
  ...
  // Default values
  AFLplusplusFuzzerOptions() : forksrv(true), frida_mode(false){};
};
...
std::unique_ptr<TFuzzer> BuildAFLplusplusFuzzerFromArgs(
  ...
  fuzzer_desc.add_options()(
      ...
      ("det,D",
       "Do not skip deterministic stages if specified (AFLplusplus skips them by default)");
  ...
  if (global_options.help) {
    fuzzuf::cli::fuzzer::aflplusplus::usage(fuzzer_args.global_options_description);
  }
  ...
  auto setting = std::make_shared<const AFLplusplusSetting>(
      ...
      global_options.exec_timelimit_ms.value_or(GetExecTimeout<AFLplusplusTag>()),
  ...
  switch (global_options.executor) {
    case ExecutorKind::NATIVE: {
      auto nle = std::make_shared<fuzzuf::executor::NativeLinuxExecutor>(
          ...
          setting->forksrv, setting->out_dir / GetDefaultOutfile<AFLplusplusTag>(),
    ...
    case ExecutorKind::QEMU: {
      // NOTE: Assuming GetMapSize<AFLplusplusTag>() == QEMUExecutor::QEMU_SHM_SIZE
