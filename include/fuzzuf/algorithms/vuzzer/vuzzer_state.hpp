...
struct VUzzerState {
    ...
    bool errorbb = true;                    /* this flag decides if we want to run error BB detection step. */
    bool has_new_cov = false;               /* this flag is set on when new seeds which records new coverage appear */
    ...
    /* Fuzzing queues */
    std::vector<std::shared_ptr<VUzzerTestcase>> seed_queue; // seeds which have already executed and evaluated.
    std::vector<std::shared_ptr<VUzzerTestcase>> pending_queue; // seeds which have not evaluated yet
    std::vector<std::shared_ptr<VUzzerTestcase>> keep_queue; // seeds which should be keeped
    std::vector<std::shared_ptr<VUzzerTestcase>> taint_queue; // seeds which should be executed by taint engine
