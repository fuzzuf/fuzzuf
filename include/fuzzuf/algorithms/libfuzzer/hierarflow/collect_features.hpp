 ...
 * @brief Calculate features of specified execution result.
...
struct CollectFeatures<R(Args...), Path>
 ...
 public:
   ...
   * offset to the corresponding byte in the coverage. In some cases, there are
   * multiple coverage arrays. For example, executing multiple targets or there
   * are dynamic linked library that has dedicated coverage array for each
   * libraries. In such case, module_offset need to be set to avoid conflict of
   * feature IDs.
   */
