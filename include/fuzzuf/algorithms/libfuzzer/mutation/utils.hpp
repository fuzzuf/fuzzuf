...
template <typename T> using RemoveCvrT = utils::type_traits::RemoveCvrT<T>;

template <typename T> using bswap = utils::bswap<T>;

template <typename E>
auto IncrementUseCount(E &e)
...
template <typename E>
auto get_word(const E &e) -> std::enable_if_t<
    !(has_IncrementUseCount1_v<E> && has_get_word1_v<E>)&&!(
...
auto ApplyDictionaryEntry(RNG &rng, R &data, E &dict_entry,
  ...
  if (random_value<bool>(rng)) {
    if (size + utils::range::rangeSize(word) > max_size)
      return 0u;
    ...
    if (utils::range::rangeSize(word) > size)
      return 0u;
...
auto ChangeBinaryInteger(RNG &rng, Range &data)
  ...
  if (offset < 64u && !random_value(rng, 3u)) {
    ...
    if (random_value<bool>(rng))
      ...
      value = value + add;
    if (add == 0 || random_value<bool>(rng))
      value = -value;
  }
