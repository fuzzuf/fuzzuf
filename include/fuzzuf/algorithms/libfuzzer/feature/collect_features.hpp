...
template <typename Cov, typename Enable = void> struct CoverageDepth {};
 ...
 * @param cb Callable with one integer argument that indicates ID of detected feature.
 */
    ...
    -> std::enable_if_t<
        is_state_v<State> && coverage_depth_v<Cov> == 1u &&
            std::is_void_v<utils::void_t<decltype(
                std::declval<Callback>()(std::declval<std::uint32_t>()))>>,
        std::size_t> {
