...
constexpr std::array<double, fuzzuf::mutator::NUM_CASE> AFLGetCaseWeights(
    bool has_extras, bool has_aextras);
...
// static_assert throws error if InitCountClasses() cannot be evaluated at
// compile time.
static_assert(
    true
...
constexpr std::array<double, fuzzuf::mutator::NUM_CASE> AFLGetCaseWeights(
    ...
    bool has_extras, bool has_a_extras) {
  std::array<double, fuzzuf::mutator::NUM_CASE> weights{};

  // We use constexpr + assignment, instead of designated initialization of C
  // lang This allows us to easily assign and check the weight of each case in
  // havoc.

  weights[fuzzuf::mutator::FLIP1] = 2.0;
  weights[fuzzuf::mutator::XOR] = 2.0;

  weights[fuzzuf::mutator::DELETE_BYTES] = 4.0;  // case 11 ... 12

  weights[fuzzuf::mutator::CLONE_BYTES] = 1.5;       // UR(4) != 0 in case 13
  weights[fuzzuf::mutator::INSERT_SAME_BYTE] = 0.5;  // UR(4) == 0 in case 13

  // The following two cases are the same as the above two cases
  weights[fuzzuf::mutator::OVERWRITE_WITH_CHUNK] = 1.5;
  weights[fuzzuf::mutator::OVERWRITE_WITH_SAME_BYTE] = 0.5;

  weights[fuzzuf::mutator::INT8] = 2.0;
  weights[fuzzuf::mutator::INT16_LE] = 1.0;  // UR(2) == 1 in case 2
  weights[fuzzuf::mutator::INT16_BE] = 1.0;  // UR(2) == 0 in case 2
  weights[fuzzuf::mutator::INT32_LE] = 1.0;  // UR(2) == 1 in case 3
  weights[fuzzuf::mutator::INT32_BE] = 1.0;  // UR(2) == 0 in case 3

  // SUB and ADD are the same as INT
  weights[fuzzuf::mutator::SUB8] = 2.0;
  weights[fuzzuf::mutator::SUB16_LE] = 1.0;
  weights[fuzzuf::mutator::SUB16_BE] = 1.0;
  weights[fuzzuf::mutator::SUB32_LE] = 1.0;
  weights[fuzzuf::mutator::SUB32_BE] = 1.0;

  weights[fuzzuf::mutator::ADD8] = 2.0;
  weights[fuzzuf::mutator::ADD16_LE] = 1.0;
  weights[fuzzuf::mutator::ADD16_BE] = 1.0;
  weights[fuzzuf::mutator::ADD32_LE] = 1.0;
  weights[fuzzuf::mutator::ADD32_BE] = 1.0;

  if (has_extras && has_a_extras) {
    weights[fuzzuf::mutator::INSERT_EXTRA] = 1.0;
    weights[fuzzuf::mutator::OVERWRITE_WITH_EXTRA] = 1.0;
    weights[fuzzuf::mutator::INSERT_AEXTRA] = 1.0;
    weights[fuzzuf::mutator::OVERWRITE_WITH_AEXTRA] = 1.0;
  } else if (has_extras) {
    weights[fuzzuf::mutator::INSERT_EXTRA] = 2.0;
    weights[fuzzuf::mutator::OVERWRITE_WITH_EXTRA] = 2.0;
  } else if (has_a_extras) {
    weights[fuzzuf::mutator::INSERT_AEXTRA] = 2.0;
    weights[fuzzuf::mutator::OVERWRITE_WITH_AEXTRA] = 2.0;
  }
