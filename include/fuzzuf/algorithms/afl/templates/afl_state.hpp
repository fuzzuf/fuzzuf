...
InplaceMemoryFeedback AFLStateTemplate<Testcase>::RunExecutorWithClassifyCounts(
    const u8* buf, u32 len, ExitStatusFeedback& exit_status, u32 tmout) {
  total_execs++;
  ...
  std::cout << __FILE__ << " " << __LINE__ << " "
            << inp_feed.CountNonZeroBytes() << std::endl;
  exit_status = executor->GetExitStatusFeedback();
  std::cout << __FILE__ << " " << __LINE__ << " "
            << int(exit_status.exit_reason) << " " << int(exit_status.signal)
            << std::endl;
...
PUTExitReasonType AFLStateTemplate<Testcase>::CalibrateCaseWithFeedDestroyed(
    Testcase& testcase, const u8* buf, u32 len, InplaceMemoryFeedback& inp_feed,
    ExitStatusFeedback& exit_status, u32 handicap, bool from_queue) {
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    std::cout << __FILE__ << " " << __LINE__ << " " << stage_cur << std::endl;
    ...
    std::cout << __FILE__ << " " << __LINE__ << " " << setting->dumb_mode << " "
              << stage_cur << " " << inp_feed.CountNonZeroBytes() << std::endl;
...
void AFLStateTemplate<Testcase>::UpdateBitmapScore(
    Testcase& testcase, const InplaceMemoryFeedback& inp_feed) {
  inp_feed.ShowMemoryToFunc(
...
bool AFLStateTemplate<Testcase>::SaveIfInteresting(
    const u8* buf, u32 len, InplaceMemoryFeedback& inp_feed,
...
void AFLStateTemplate<Testcase>::WriteStatsFile(double bitmap_cvg,
  ...
  fprintf(
      ...
      (qemu_mode || setting->dumb_mode || no_forkserver ||
       crash_mode != PUTExitReasonType::FAULT_NONE || persistent_mode ||
       deferred_mode)
          ? ""
          : "default",
  ...
  for (const auto& testcase : case_queue) {
    ...
    PUTExitReasonType res = CalibrateCaseWithFeedDestroyed(
        *testcase, input.GetBuf(), input.GetLen(), inp_feed, exit_status, 0,
        true);
    std::cout << __FILE__ << " " << __LINE__ << " " << int(res) << std::endl;
...
void AFLStateTemplate<Testcase>::ShowStats(void) {
  ...
  if (!setting->dumb_mode &&
      (last_path_time || resuming_fuzz || queue_cycle == 1 ||
       !in_bitmap.empty() || crash_mode != PUTExitReasonType::FAULT_NONE)) {
