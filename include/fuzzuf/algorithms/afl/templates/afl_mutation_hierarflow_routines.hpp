...
AFLMutCalleeRef<State> BitFlip1WithAutoDictBuildTemplate<State>::operator()(
  ...
  for (state.stage_cur = 0; state.stage_cur < state.stage_max;
    ...
    // If the following conditions are met, consider appending Auto Extra values
    // in the successor For the details, check
    // fuzzuf::algorithm::afl::routine::update::ConstructAutoDict
...
template <class State>
BitFlipOtherTemplate<State>::BitFlipOtherTemplate(State &state)
    : state(state) {}
...
AFLMutCalleeRef<State> BitFlipOtherTemplate<State>::operator()(
  ...
  int stage_idxs[] = {option::STAGE_FLIP2, option::STAGE_FLIP4};
...
AFLMutCalleeRef<State> ByteFlip1WithEffMapBuildTemplate<State>::operator()(
  ...
  if (state.eff_cnt != EFF_ALEN<Tag>(mutator.GetLen()) &&
      state.eff_cnt * 100 / EFF_ALEN<Tag>(mutator.GetLen()) >
          option::GetEffMaxPerc(state)) {
...
AFLMutCalleeRef<State> ByteFlipOtherTemplate<State>::operator()(
  ...
  int stage_idxs[] = {option::STAGE_FLIP16, option::STAGE_FLIP32};
...
AFLMutCalleeRef<State> UserDictInsertTemplate<State>::operator()(
  ...
  std::unique_ptr<u8[]> ex_tmp(
      new u8[mutator.GetLen() + option::GetMaxDictFile(state)]);
  for (u32 i = 0; i <= mutator.GetLen(); i++) {
    ...
    for (u32 j = 0; j < state.extras.size(); j++) {
      ...
      /* Insert token */
      std::memcpy(ex_tmp.get() + i, &state.extras[j].data[0],
                  state.extras[j].data.size());

      /* Copy tail */
      std::memcpy(ex_tmp.get() + i + state.extras[j].data.size(),
                  mutator.GetBuf() + i, mutator.GetLen() - i);

      if (this->CallSuccessors(
              ex_tmp.get(), mutator.GetLen() + state.extras[j].data.size())) {
        this->SetResponseValue(true);
        return this->GoToParent();
      }
...
template <class State>
HavocBaseTemplate<State>::HavocBaseTemplate(State &state) : state(state) {}
...
bool HavocBaseTemplate<State>::DoHavoc(
  ...
  for (state.stage_cur = 0; state.stage_cur < state.stage_max;
    ...
    if (unlikely(havoc_finds_diff > 0) && useHavocOperatorFinds) {
      ...
      fuzzuf::optimizer::Store::GetInstance().Set(
          fuzzuf::optimizer::keys::HavocOperatorFinds, havoc_operator_finds);
    }
