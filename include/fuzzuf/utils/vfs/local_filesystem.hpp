...
public:
  /**
   * Constructor.
   * @param allowed_path_ Only paths listed here are accessible from the VFS. For more detail check fuzzuf::utils::vfs::VFS.
   * @param create If true, directories are created if the directory specified by allowed_path_ is not exist. Otherwise, invalid_argument exception is thrown if not exist.
   */
  ...
  /**
   * Copy constructor.
   * LocalFilesystem is copyable. If copied, both instance share the allowed paths yet have independent current directory.
   */
...
#else
  virtual std::chrono::system_clock::time_point LastWriteTime(const fs::path &p) override;
#endif
  /**
   * wrapper to fs::space
   ...
   * @param populate If true, load contents immediately. Otherwise, load contents on demand.
   */
  virtual mapped_file_t Mmap( const fs::path &p, unsigned int flags, bool populate ) override;
  /**
