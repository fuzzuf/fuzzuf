...
template <std::size_t i, typename R> nth_range<i, R> make_nth_range(R &v) {
  using iterator =
...
namespace adaptor {
template <size_t i> struct nth_t {};

template <size_t i> constexpr nth_t<i> nth;

template <size_t i, typename T> auto operator|(T &p, const nth_t<i> &) {
