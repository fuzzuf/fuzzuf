...
BOOST_AUTO_TEST_CASE(IJONExecute) {
  ...
  executor.GetAFLFeedback().ShowMemoryToFunc(
      [ijon_counter_offset, ijon_max_offset](const u8 *head, u32 size) {
        ...
        BOOST_CHECK_EQUAL( std::size_t( std::count( std::next( head, ijon_counter_offset ), std::next( head, ijon_counter_offset + 65536u ), 0 ) ), 65536u - 2u );
        // At least one IJON_MAX value is recorded
        BOOST_CHECK_EQUAL( std::size_t( std::count( std::next( head, ijon_max_offset ), std::next( head, ijon_max_offset + 65536u ), 0 ) ), 65536u - 8u );
      });
}
