 ...
 */
#define BOOST_TEST_MODULE coresight_executor.run
...
// CoreSightExecutor::Run() の正常系テスト
BOOST_AUTO_TEST_CASE(CoreSightExecutorRun) {
  ...
  fuzzuf::executor::CoreSightExecutor executor(
      fs::path(FUZZUF_CS_PROXY_EXECUTABLE),
      {"/usr/bin/tee", output_file_path.native()}, 1000, 10000, true,
      path_to_write_seed, (1U << 16), true /* record_stdout_and_err */
  );
  BOOST_CHECK_EQUAL(executor.stdin_mode, true);

  // Invoke CoreSightExecutor::Run()
  std::string input("abcd");
  executor.Run(reinterpret_cast<const u8 *>(input.c_str()), input.size());

  // Check normality
  // (1) 正常実行されたこと → feedbackのexit_reason が
  // feedback::PUTExitReasonType::FAULT_NONE であることを確認する
  ...
  // Ignore stderr output for CoreSightExecutor because the tracer often outputs
  // debug messages with memory map info.
#if 0
