...
// via an argument moreover, "whether AFL coverage can be retrieved" should be
// independent from need_bb_cov. so we test both cases, need_bb_cov=false,
// need_bb_cov=true
static void InstrumentCovAFL(bool forksrv, bool need_bb_cov) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    // moreover, normally the difference should be exactly two edges(where "var
    // = 1" or "var = 0" takes place)
...
BOOST_AUTO_TEST_CASE(Test4CasesWithInstrumentCovAFL) {
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 2; j++) {
      ...
      if (j)
        std::cout << ", need_bb_cov=true" << std::endl;
      else
        std::cout << ", need_bb_cov=false" << std::endl;
