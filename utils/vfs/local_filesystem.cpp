...
#else
std::chrono::system_clock::time_point LocalFilesystem::LastWriteTime(const fs::path& p) {
  return std::chrono::system_clock::from_time_t( fs::last_write_time( SanitizePath( p ) ) );
#endif
}
